...
将 [[HaWoR]]/[[WiLoR]] 的 3D 手部重建能力用于**第一人称视频中的手势意图理解**——当用户戴着智能眼镜指着一个物体问"What is that?"，模型需要理解"指"这个手势指向的目标。
...
- **手势意图流**：[[WiLoR]] 每帧提取 21 个 3D 手部关键点 → **Keypoint Adapter**（两层 MLP + GeLU）→ **HINT token**
...
- [[HaWoR]] — 世界坐标系手部重建（CVPR 2025 Highlight），本工作的底层手部重建能力来源
- [[WiLoR]] — 单图 3D 手部检测+重建 backbone，HINT 直接使用其关键点输出
- [[Dyn-HaMR]] — 平行互补的双手全局运动重建

## 思考

这是 [[HaWoR]] 的"手部重建能力→高层语义理解"的**直接应用延伸**。说明 3D 手部重建不是终点，而是具身 AI 助手的感知基础设施。Meta Ray-Ban 智能眼镜作为采集设备也暗示了 AR 眼镜作为未来人机交互平台的技术路径。
